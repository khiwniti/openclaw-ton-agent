    ...
    async run(state: GramTradeState): Promise<GramTradeState> {
      ...
      for (let step = 0; step < 20; step++) {
        ...
        if (next === "supervisor") {
          ...
          const out = await supervisorNode(current, async (_tier) => ({
            balance_ton: ctx.tier.balanceTon,
            open_positions: current.open_positions,
            max_position_ton: ctx.tier.maxPositionTon,
            max_open: ctx.tier.maxOpen,
          }))
          current = {
            ...current,
            ...(out.state as any),
            discarded: current.discarded || out.state.discarded || false,
            discard_reason: out.state.discard_reason || current.discard_reason,
          }
